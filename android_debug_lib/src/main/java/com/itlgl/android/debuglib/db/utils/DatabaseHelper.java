...
package com.itlgl.android.debuglib.db.utils;
...
public class DatabaseHelper {
    ...
    public static TableDataResponse execSql(SQLiteDB db, String sql) {
        TableDataResponse tableData = new TableDataResponse();

        if(sql == null || "".equals(sql)) {
            tableData.isSuccessful = false;
            tableData.errorMessage = "sql is null";
            return tableData;
        }

        sql = sql.trim();
        String sqlType = getFirstWord(sql).toUpperCase();
        if("SELECT".equals(sqlType) || "PRAGMA".equals(sqlType) || "EXPLAIN".equals(sqlType)) {
            return getTableData(db, sql, null);
        } else {
//            return exec(db, sql);
            try {
                db.execSQL(sql);
                ...
                tableInfo.title = "exec sql";
                tableData.tableInfos.add(tableInfo);

                tableData.rows = new ArrayList<>();
                List<TableDataResponse.ColumnData> cdList = new ArrayList<>();
                TableDataResponse.ColumnData cd = new TableDataResponse.ColumnData();
                cd.value = "exec sql success";
                cdList.add(cd);
                tableData.rows.add(cdList);
                return tableData;
            } catch (Exception e) {
                e.printStackTrace();
                tableData.isSuccessful = false;
